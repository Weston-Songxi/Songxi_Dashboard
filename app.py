...
def get_price_history(tickers, start_date):
    ...
    with st.spinner("🔄 同步 TMT 市场数据..."):
        try:
            data = _download_price_history(tuple(all_tickers), str(pd.to_datetime(start_date).date()))
            if not data.empty:
...
def calculate_full_history(df_trans, price_data, sys_start_date):
    ...
    # 行情缺失兜底：使用最近成交价估值，避免把持仓直接估成 0
    fallback_prices = (
        df_trans[df_trans["Ticker"] != "CASH"]
        .sort_values("Date")
    ...
    for d in full_dates:
        ...
        for t, s in holdings.items():
            ...
            px = np.nan
            if (not price_data.empty) and (t in price_data.columns) and (d_norm in price_data.index):
                px = price_data.at[d_norm, t]
            if pd.isna(px):
                px = fallback_prices.get(t, np.nan)

            if pd.notna(px):
                mkt_val += s * float(px)

        spy_val = np.nan
        if (not price_data.empty) and ("SPY" in price_data.columns) and (d_norm in price_data.index):
            spy_val = float(price_data.at[d_norm, "SPY"])
...
# 注意：这里不再重复拉行情和重算，复用初始化阶段结果，减少 API 请求
...
html_parts.append(
    f'<div class="kpi-box" style="{style_exp}"><div class="kpi-label" style="color:#6c757d">净多头仓位</div><div class="kpi-value" style="color:{color_exp}">{net_exp_val:.1f}%</div></div>'
)
html_parts.append(
    f'<div class="kpi-box" style="{s_1w}"><div class="kpi-label" style="color:{l_1w}">近一周</div><div class="kpi-value" style="color:{c_1w}">{t_1w}</div></div>'
)
html_parts.append(
    f'<div class="kpi-box" style="{s_1m}"><div class="kpi-label" style="color:{l_1m}">近一月</div><div class="kpi-value" style="color:{c_1m}">{t_1m}</div></div>'
)
html_parts.append(
    f'<div class="kpi-box" style="{s_1y}"><div class="kpi-label" style="color:{l_1y}">近一年</div><div class="kpi-value" style="color:{c_1y}">{t_1y}</div></div>'
)
